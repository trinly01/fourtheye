    ...
    getData () {
      if (this.$store.state.user && this.$store.state.user.data) {
        ...
        this.$database.ref('companies').orderByChild('code').equalTo(this.$store.state.user.data.company.code).once('value', (snapshot) => {
          console.log(`companies/${Object.keys(snapshot.val())[0]}/history`)
          this.$bindAsArray('history', this.$database.ref(`companies/${Object.keys(snapshot.val())[0]}/history`).orderByChild('date'))
